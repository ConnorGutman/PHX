...
const MetroElementView = ({
  ...
  // Get this Metro Element from all elements
  let metroElement = null;
  if (metroElementId && metroElements) {
    metroElement = metroElements.find(item => parseInt(item.id, 10) === parseInt(metroElementId, 10));
  }

  // Element not found
  if (!metroElement) return <Error content={ErrorMessages.metro404} />;
